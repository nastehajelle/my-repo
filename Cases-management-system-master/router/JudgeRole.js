...
const JudgeRole = require('../models/JudgeRole');
...
router.get('/JudgeRoleDashboard', async (req, res) => {
     try {
          ...
          Cases.aggregate([
               ...
               {
                    $match: {
                         'Status': { $in: ['Procecutor', 'CLOSED'] }, // Match cases by status
                    },
               },
               {
                    $lookup: {
               ...
               //Proscuter roles aa inta ksoo qabanoya
               {
                    $lookup: {
                         ...
                         localField: 'CaseReview.procecutorId', // Use lawyerid from CaseReview
                         foreignField: '_id', // Match with _id field in lawyers collection
               ...
               {
                    $lookup: {
                         from: 'judgeroles', // Collection containing lawyer details
                         localField: '_id', // Use lawyerid from CaseReview
                         foreignField: 'CaseId', // Match with _id field in lawyers collection
                         as: 'judgeroles', // Alias for lawyer details
                    },
               },
               {
                    $unwind: {
...
router.post('/AddJudgeRole', upload.single('Attachments'), async (req, res) => {

     try {
          const NewJudgeRole = new JudgeRole({
               CaseId: req.body.CaseId,
               Description: req.body.Discription,
               XUKUN: req.body.XUKUN,
          });

          const Status = "CLOSED";
          console.log("Case", NewJudgeRole);


          const IsJudgeRoleSaved = await NewJudgeRole.save();
          ...
          if (IsJudgeRoleSaved ) {
               ...
               if (IsStatusUpdated){
                    ...
                    res.redirect('/JudgeRoleDashboard')
               }else{
